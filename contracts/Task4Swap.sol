...
contract Task4Swap is ExamBase {
    ...
    function swapExactIn(bool zeroForOne, uint256 amountIn) external returns (int256 amount0, int256 amount1) {
        ...
        require(predictionRecorded == true, "record your prediction before you swap"); // predictionRecorded must be true, else send an error msg
        ...
        int256 amountSpecified = -int256(amountIn); // cast amountIn to int256, and make it negative
        ...
        // if we're selling currency0 for currency1 (zeroForOne == true), price is going down, therefore limit is TickMath.MIN_SQRT_PRICE + 1, else limit isTickMath.MAX_SQRT_PRICE - 1
